#!/usr/bin/env python3
"""Pluggable credential storage for aircall-mcp.
...
Storage order (read):  OS keyring  ->  process env  ->  ~/.aircall-mcp/.env file
...
user opts out of keyring with ``AIRCALL_MCP_USE_KEYRING=0``.
...
SERVICE_NAME = "aircall-mcp"
CONFIG_DIR = Path.home() / ".aircall-mcp"
ENV_FILE = CONFIG_DIR / ".env"
_USE_KEYRING_FLAG = "AIRCALL_MCP_USE_KEYRING"
# ---------------------------------------------------------------------------
...
def _keyring_enabled() -> bool:
    ...
    Opt out with ``AIRCALL_MCP_USE_KEYRING=0`` (or false/no/off). A failing or
